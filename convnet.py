...
class NetFunction:
    ...
    def get_policy_value_for_mcts(self, board_info: Foundation):
        avail_move_lists = board_info.non_occupied_moves
        # input:size-(4,8,8)
        # Error:some of the strides of a given numpy array are negative... -> np.ascontiguousarray
        state_planes = torch.from_numpy(np.ascontiguousarray(board_info.getFeaturePlane)).unsqueeze(dim=0)  # NCHW 增加一个假维度
