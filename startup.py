...
if __name__ == "__main__":
    ...
    # 初始化棋盘数据(棋盘大小, 输赢判断等)
    board_obj = basics1.Foundation(board_size=config.GUI_BOARD_SIZE)
    # 初始化游戏数据(gui界面实现,人机对弈,自我训练等)
    game_obj = basics1.Game(board_obj)
    # 创建神经网络用于辅助AI的mcts的部分搜索过程（二选一）
    if config.AI_NET_TYPE == 'cnn':
        net = convnet.NetFunction(config.GUI_BOARD_SIZE-1, model_path=cnn_model_path)  # 普通cnn网络
    elif config.AI_NET_TYPE == 'resnet':
        net = resnet.NetFunction(config.GUI_BOARD_SIZE-1, model_path=resnet_model_path)  # resnet网络
    else:
        net = None
    # 创建AI
